...
| `warpImage.m` | `geometry/sampling.py` | Backward bilinear sampling | Exact primitive |
...
| `laplacian_pyramid.m` | `multiscale/laplacian.py` | Paper six-band default; source five-band private profile | Exact per profile |
| `gaussian_pyramid.m` | `multiscale/masked_gaussian.py` | Normalized mask-aware separable Gaussian | Corrected/extended |
| paper energy equations | `multiscale/energy.py` | Reference energy before warp in production | Exact equation |
...
| `RGB2Lab.m`, `Lab2RGB.m` | `color/lab.py`, `color/legacy_color.py` | Standard sRGB/D65 production; legacy ranges private | Corrected and isolated |
