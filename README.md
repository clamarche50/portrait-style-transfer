...
Portrait Style Transfer is a privacy-conscious web application that uses one headshot as structure/content and a second portrait as the visual-style reference. The single production profile, `ai_instantstyle_v1`, runs locally on an isolated GPU service: Stable Diffusion XL 1.0 supplies the generative backbone, InstantStyle injects the reference appearance through style-sensitive attention layers, and IP-Adapter FaceID PlusV2 with InsightFace embeddings preserves the subject's identity.

The deterministic analysis layer (preflight, MediaPipe face analysis, segmentation, quality scoring, and reference style ranking) remains in `packages/portrait_transfer`. The former classical multiscale transfer pipeline has been removed. AI output is synthesized and can change identity details despite identity conditioning. Treat every result as an edited/generated image, not a factual photograph.
...
- `services/ai_engine/`: internal-only InstantStyle GPU inference API.
- `docs/`: architecture, algorithm, operations, privacy, and licensing records.
- `scripts/`: model management, cleanup, and smoke tools.
...
- At least 30 GB of free Docker disk for the 13 GiB model tree, multi-gigabyte
...
make models-instantstyle        download only the SDXL/InstantStyle/FaceID artifacts
...
Standard CPU CI uses `compose.ci.yml`, which explicitly replaces InstantStyle with a
...
`models/manifest.json` pins the MediaPipe analysis assets. `models/instantstyle/manifest.json` records the SDXL base, InstantStyle IP-Adapter, FaceID PlusV2, and InsightFace runtime files by exact byte length and SHA-256. Hugging Face artifacts use immutable commit revisions. For air-gapped deployment, provision both trees and verify without network access:
...
Model binaries are ignored, excluded from Docker build contexts, mounted read-only at runtime, and never downloaded in a request. Review [the model notices](THIRD_PARTY_NOTICES.md) before any redistribution; the InsightFace models carry non-commercial research terms.

## Tests and builds

Required CI uses synthetic images and mocked model adapters; no real face is committed:
...
Do not interpret a mocked-CV smoke test as validation of real model quality.
...
Xiaoxiao Wu et al. “InstantStyle: Free Lunch towards Style-Preserving in Text-to-Image Generation.” 2024, arXiv:2404.02733. Hu Ye et al. “IP-Adapter: Text Compatible Image Prompt Adapter for Text-to-Image Diffusion Models.” 2023, arXiv:2308.06721. Stability AI, “Stable Diffusion XL 1.0.” InsightFace, buffalo_l face-analysis pack. See [algorithm details](docs/algorithm.md) and [third-party notices](THIRD_PARTY_NOTICES.md).

YiChang Shih, Sylvain Paris, Connelly Barnes, William T. Freeman, and Frédo Durand. “Style Transfer for Headshot Portraits.” *ACM Transactions on Graphics* 33(4), SIGGRAPH 2014. The retained analysis layer (style energy ranking and preprocessing conventions) is informed by this paper; the engine does not claim bitwise or perceptual equivalence to its published figures.
